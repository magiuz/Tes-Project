...
$panjangword1 = strlen($word1);//$panjangword1 untuk ngitung jumlah karaternya ada berapa baru habi itu di simpen di var $tampung
$tampung = "";
    for($i=($panjangword1-1); $i>=0; $i--){ //karna array itu mulai dari index 0 supaya bisa itung index dari 0 karna strlen mulai dari 1 dari panjang kata nya
        $tampung .= $word1[$i];
    }
    return $tampung;
}
...
// Code function di sini
function palindrome($word3){
    $balik = reverse($word3);
        if($word3 === $balik){
            echo "Palindrome ". $word3 . " => True <br>";
        }else{
            echo "Palindrome ". $word3 . " => False <br>";
        }
...
// Code function di sini
function tentukan_nilai($no){
    $output = "";
    if($no >= 85 && $no <= 100){
        $output .= "Nilai " . $no ." => Sangat Baik <br>";
    }else if($no >= 70 && $no < 85){
        $output .= "Nilai " . $no . " => Baik <br>";
    }else if($no >= 60 && $no < 70){
        $output .= "Nilai " . $no . " => Cukup <br>";
    }else{
        $output .= "Nilai " . $no . " => Kurang";
    }
    return $output;
}
